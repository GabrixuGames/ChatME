.TH "frontend/node_modules/minipass Directory Reference" 3 "Version 0.1" "Chat-ME!" \" -*- nroff -*-
.ad l
.nh
.SH NAME
frontend/node_modules/minipass Directory Reference
.SH SYNOPSIS
.br
.PP
...
.PP
There is a \fRread()\fP method, but it's much more efficient to consume data from this stream via \fR'data'\fP events or by calling \fRpipe()\fP into some other stream\&. Calling \fRread()\fP requires the buffer to be flattened in some cases, which requires copying memory\&.
...
.PP
\fRobjectMode\fP can only be set at instantiation\&. Attempting to write something other than a String or Buffer without having set \fRobjectMode\fP in the options will throw an error\&.
...
\fRminipass-sized\fP
.PP
.SH "Usage in TypeScript"
.PP
The \fRMinipass\fP class takes three type template definitions:

.PP
.IP "\(bu" 2
\fRRType\fP the type being read, which defaults to \fRBuffer\fP\&. If \fRRType\fP is \fRstring\fP, then the constructor \fImust\fP get an options object specifying either an \fRencoding\fP or \fRobjectMode: true\fP\&. If it's anything other than \fRstring\fP or \fRBuffer\fP, then it \fImust\fP get an options object specifying \fRobjectMode: true\fP\&.
.IP "\(bu" 2
\fRWType\fP the type being written\&. If \fRRType\fP is \fRBuffer\fP or \fRstring\fP, then this defaults to \fRContiguousData\fP (Buffer, string, ArrayBuffer, or ArrayBufferView)\&. Otherwise, it defaults to \fRRType\fP\&.
.IP "\(bu" 2
\fREvents\fP type mapping event names to the arguments emitted with that event, which extends \fRMinipass\&.Events\fP\&.
.PP
...
.PP
Emitting/handling events that aren't declared in this way is fine, but the arguments will be typed as \fRunknown\fP\&.
...
One hazard of immediately emitting \fR'end'\fP is that you may not yet have had a chance to add a listener\&. In order to avoid this hazard, Minipass streams safely re-emit the \fR'end'\fP event if a new listener is added after \fR'end'\fP has been emitted\&.

.PP
Ie, if you do \fRstream\&.on('end', someFunction)\fP, and the stream has already emitted \fRend\fP, then it will call the handler right away\&. (You can think of this somewhat like attaching a new \fR\&.then(fn)\fP to a previously-resolved Promise\&.)

.PP
To prevent calling handlers multiple times who would not expect multiple ends to occur, all listeners are removed from the \fR'end'\fP event whenever it is emitted\&.
.SS "Emit \fRerror\fP When Asked"
The most recent error object passed to the \fR'error'\fP event is stored on the stream\&. If a new \fR'error'\fP event handler is added, and an error was previously emitted, then the event handler will be called immediately (or on \fRprocess\&.nextTick\fP in the case of async streams)\&.
...
.PP
The same caveat applies to \fRon('data')\fP event listeners\&. The first one added will \fIimmediately\fP receive all of the data, leaving nothing for the second:
...
.IP "\(bu" 2
\fRsignal\fP An \fRAbortSignal\fP that will cause the stream to unhook itself from everything and become as inert as possible\&. Note that providing a \fRsignal\fP parameter will make \fR'error'\fP events no longer throw if they are unhandled, but they will still be emitted to handlers if any are attached\&.
.PP
.SS "API"
...
.IP "  \(bu" 4
\fRoptions\&.proxyErrors\fP - Boolean, proxy \fRerror\fP events from the source stream to the destination stream\&. Note that errors are \fInot\fP proxied after the pipeline terminates, either due to the source emitting \fR'end'\fP or manually unpiping with \fRsrc\&.unpipe(dest)\fP\&. Default \fRfalse\fP\&.
.PP

.IP "\(bu" 2
\fRon(ev, fn)\fP, \fRemit(ev, fn)\fP - Minipass streams are EventEmitters\&. Some events are given special treatment, however\&. (See below under "events"\&.)
.IP "\(bu" 2
\fRpromise()\fP - Returns a Promise that resolves when the stream emits \fRend\fP, or rejects if the stream emits \fRerror\fP\&.
.IP "\(bu" 2
\fRcollect()\fP - Return a Promise that resolves on \fRend\fP with an array containing each chunk of data that was emitted, or rejects if the stream emits \fRerror\fP\&. Note that this consumes the stream data\&.
.IP "\(bu" 2
\fRconcat()\fP - Same as \fRcollect()\fP, but concatenates the data into a single Buffer object\&. Will reject the returned promise if the stream is in objectMode, or if it goes into objectMode by the end of the data\&.
.IP "\(bu" 2
\fRread(n)\fP - Consume \fRn\fP bytes of data out of the buffer\&. If \fRn\fP is not provided, then consume all of it\&. If \fRn\fP bytes are not available, then it returns null\&. \fBNote\fP consuming streams in this way is less efficient, and can lead to unnecessary Buffer copying\&.
.IP "\(bu" 2
\fRdestroy([er])\fP - Destroy the stream\&. If an error is provided, then an \fR'error'\fP event is emitted\&. If the stream has a \fRclose()\fP method, and has not emitted a \fR'close'\fP event yet, then \fRstream\&.close()\fP will be called\&. Any Promises returned by \fR\&.promise()\fP, \fR\&.collect()\fP or \fR\&.concat()\fP will be rejected\&. After being destroyed, writing to the stream will emit an error\&. No more data will be emitted if the stream is destroyed, even if it was previously buffered\&.
.PP
.SS "Properties"
.IP "\(bu" 2
\fRbufferLength\fP Read-only\&. Total number of bytes buffered, or in the case of objectMode, the total number of objects\&.
.IP "\(bu" 2
\fRencoding\fP Read-only\&. The encoding that has been set\&.
.IP "\(bu" 2
...
.IP "\(bu" 2
\fRobjectMode\fP Indicates whether the stream is in \fRobjectMode\fP\&.
.IP "\(bu" 2
...
.IP "\(bu" 2
\fRprefinish\fP An end-ish event that follows the same logic as \fRend\fP and is emitted in the same conditions where \fRend\fP is emitted\&. Emitted after \fR'end'\fP\&.
.IP "\(bu" 2
\fRfinish\fP An end-ish event that follows the same logic as \fRend\fP and is emitted in the same conditions where \fRend\fP is emitted\&. Emitted after \fR'prefinish'\fP\&.
.IP "\(bu" 2
\fRclose\fP An indication that an underlying resource has been released\&. Minipass does not emit this event, but will defer it until after \fRend\fP has been emitted, since it throws off some stream libraries otherwise\&.
.IP "\(bu" 2
\fRdrain\fP Emitted when the internal buffer empties, and it is again suitable to \fRwrite()\fP into the stream\&.
.IP "\(bu" 2
\fRreadable\fP Emitted when data is buffered and ready to be read by a consumer\&.
.IP "\(bu" 2
\fRresume\fP Emitted when stream changes state from buffering to flowing mode\&. (Ie, when \fRresume\fP is called, \fRpipe\fP is called, or a \fRdata\fP event listener is added\&.)
.PP
.SS "Static Methods"
.IP "\(bu" 2
