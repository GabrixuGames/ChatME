.TH "frontend/node_modules/proxy-from-env Directory Reference" 3 "Version 0.1" "Chat-ME!" \" -*- nroff -*-
.ad l
.nh
.SH NAME
frontend/node_modules/proxy-from-env Directory Reference
.SH SYNOPSIS
.br
.PP
...
.PP
\fRproxy-from-env\fP is a Node\&.js package that exports a function (\fRgetProxyForUrl\fP) that takes an input URL (a string or \fR\fRurl\&.parse\fP\fP's return value) and returns the desired proxy URL (also a string) based on standard proxy environment variables\&. If no proxy is set, an empty string is returned\&.
...
.SH "Example"
.PP
This example shows how the data for a URL can be fetched via the \fR\fRhttp\fP module\fP, in a proxy-aware way\&.
...
.SS "NO_PROXY"
\fRNO_PROXY\fP is a list of host names (optionally with a port)\&. If the input URL matches any of the entries in \fRNO_PROXY\fP, then the input URL should be fetched by a direct request (i\&.e\&. without a proxy)\&.
...
.PP
.IP "\(bu" 2
\fRNO_PROXY=*\fP disables all proxies\&.
.IP "\(bu" 2
Space and commas may be used to separate the entries in the \fRNO_PROXY\fP list\&.
.IP "\(bu" 2
If \fRNO_PROXY\fP does not contain any entries, then proxies are never disabled\&.
.IP "\(bu" 2
...
.IP "\(bu" 2
Generally, the proxy is only disabled if the host name is an exact match for an entry in the \fRNO_PROXY\fP list\&. The only exceptions are entries that start with a dot or with a wildcard; then the proxy is disabled if the host name ends with the entry\&.
.PP

.PP
See \fRtest\&.js\fP for examples of what should match and what does not\&.
.SS "*_PROXY"
The environment variable used for the proxy depends on the protocol of the URL\&. For example, \fRhttps://example.com\fP uses the "https" protocol, and therefore the proxy to be used is \fRHTTPS_PROXY\fP (\fINOT\fP \fRHTTP_PROXY\fP, which is \fIonly\fP used for http:-URLs)\&.

.PP
The library is not limited to http(s), other schemes such as \fRFTP_PROXY\fP (ftp:), \fRWSS_PROXY\fP (wss:), \fRWS_PROXY\fP (ws:) are also supported\&.

.PP
If present, \fRALL_PROXY\fP is used as fallback if there is no other match\&.
