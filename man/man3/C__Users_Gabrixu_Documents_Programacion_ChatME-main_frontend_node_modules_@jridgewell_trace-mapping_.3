.TH "frontend/node_modules/@jridgewell/trace-mapping Directory Reference" 3 "Version 0.1" "Chat-ME!" \" -*- nroff -*-
.ad l
.nh
.SH NAME
frontend/node_modules/@jridgewell/trace-mapping Directory Reference
.SH SYNOPSIS
.br
.PP
...
.PP
\fRtrace-mapping\fP allows you to take the line and column of an output file and trace it to the original location in the source file through a source map\&.

.PP
You may already be familiar with the \fR\fRsource-map\fP\fP package's \fRSourceMapConsumer\fP\&. This provides the same \fRoriginalPositionFor\fP and \fRgeneratedPositionFor\fP API, without requiring WASM\&.
.SH "Installation"
.PP
.PP
.nf
...
.PP
We also provide a lower level API to get the actual segment that matches our line and column\&. Unlike \fRoriginalPositionFor\fP, \fRtraceSegment\fP uses a 0-base for \fRline\fP:
...
.fi
.PP
.SS "SectionedSourceMaps"
The sourcemap spec defines a special \fRsections\fP field that's designed to handle concatenation of output code with associated sourcemaps\&. This type of sourcemap is rarely used (no major build tool produces it), but if you are hand coding a concatenation you may need it\&. We provide an \fRAnyMap\fP helper that can receive either a regular sourcemap or a \fRSectionedSourceMap\fP and returns a \fRTraceMap\fP instance:
